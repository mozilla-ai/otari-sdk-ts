...
|------------- | ------------- | -------------|
| [**webSearchWebSearch**](WebSearchApi.md#websearchwebsearch) | **GET** /api/v1/web-search/search | Web Search |



## webSearchWebSearch

> WebSearchBackendResponse webSearchWebSearch(q, maxResults, searchDepth, topic, timeRange, includeAnswer, xGatewayToken)
...
import type { WebSearchWebSearchRequest } from '';
...
async function example() {
  ...
  } satisfies WebSearchWebSearchRequest;

  try {
    const data = await api.webSearchWebSearch(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
