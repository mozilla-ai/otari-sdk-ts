...
> BuiltInGuardrailCatalog toolSettingsListBuiltinGuardrails()
...
import type { ToolSettingsListBuiltinGuardrailsRequest } from '';
...
async function example() {
  ...
  try {
    const data = await api.toolSettingsListBuiltinGuardrails();
    console.log(data);
  } catch (error) {
    console.error(error);
  }
